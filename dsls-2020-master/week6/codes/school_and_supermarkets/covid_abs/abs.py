...
class Simulation(object):
    ...
    def random_position(self):
        x = int(self._xclip(self.length / 2 +
                            (np.random.randn(1) * (self.length / 3))))
        y = int(self._yclip(self.height / 2 +
    ...
    def move(self, agent, triggers=None):
        ...
        if agent.age >= 6 and agent.age <= 18 and agent.school < 5:
            agent.x = self.length-50 + np.random.randint(50)
            agent.y = self.height-40 + np.random.randint(40)
        ...
        # the supermarket has size 10x10 and the agents get assigned to a random position within the supermarket
            ...
            sm_decider = np.random.random()
            # go to big supermarket with 50% prob
            if sm_decider <= 0.5:
                agent.x = int(self.length/4-25 + np.random.randint(50))
                agent.y = int(self.height/4-20 + np.random.randint(40))
            # go to medium supermarket with 30% prob
            elif sm_decider > 0.5 and sm_decider <= 0.8:
                agent.x = int(self.length*3/4-20 + np.random.randint(40))
                agent.y = int(self.height/4-12 + np.random.randint(24))
            # go to small supermarket with 20% prob
            elif sm_decider > 0.8:
                agent.x = int(self.length/2-15 + np.random.randint(30))
                agent.y = int(self.height/1.5-10 + np.random.randint(20))
    ...
    def update(self, agent):
        ...
        big_supermarket_area_x = range(
            int(self.length/4-25), int(self.length/4+26))
        big_supermarket_area_y = range(
            int(self.height/4-20), int(self.height/4+21))
        medium_supermarket_area_x = range(
            int(self.length*3/4-20), int(self.length*3/4+21))
        medium_supermarket_area_y = range(
            int(self.height/4-13), int(self.height/4+14))
        small_supermarket_area_x = range(
            int(self.length/2-15), int(self.length/2+16))
        small_supermarket_area_y = range(
            int(self.height/1.5-10), int(self.height/1.5+11))

        if (agent.x in big_supermarket_area_x and agent.y in big_supermarket_area_y) or (agent.x in medium_supermarket_area_x and agent.y in medium_supermarket_area_y) or (agent.x in small_supermarket_area_x and agent.y in small_supermarket_area_y):
            agent.supermarket = 0
        else:
            agent.supermarket += 1
