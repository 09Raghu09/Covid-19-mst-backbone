...
def execute_simulation(sim, **kwargs):
    ...
    # show big supermarket on the plot (2000qm)
    big_supermarket = patches.Rectangle(
        (sim.length/4-25, sim.height/4-20), 50, 40, linewidth=1, edgecolor='r', facecolor='none')
    ax[0].add_patch(big_supermarket)

    # show medium supermarket on the plot (1000qm)
    medium_supermarket = patches.Rectangle(
        (sim.length*3/4-20, sim.height/4-12), 40, 25, linewidth=1, edgecolor='r', facecolor='none')
    ax[0].add_patch(medium_supermarket)

    # show small supermarket on the plot (600qm)
    small_supermarket = patches.Rectangle(
        (sim.length/2-15, int(sim.height/1.5-10)), 30, 20, linewidth=1, edgecolor='r', facecolor='none')
    ax[0].add_patch(small_supermarket)

    # show school on the plot
    school = patches.Rectangle(
        (sim.length-50, sim.height-40), 50, 40, linewidth=1, edgecolor='b', facecolor='none')
