...
class Simulation(object):
    ...
    def create_agent(self, status):
        ...
        AG = Agent(x=x, y=y, age=age, status=status, social_stratum=social_stratum)


        chance_obese        = 0.54
        chance_hypertension = 0.33
        chance_diabetes     = 0.13

        if age >= 18:
            obs_chance  =  np.random.random()
            if chance_obese >= obs_chance:
                AG.chronic.append("obs")

                if (chance_hypertension * chance_obese) >= obs_chance:
                    AG.chronic.append("hyp")

                    if(chance_hypertension * chance_obese * chance_diabetes) >= obs_chance:
                        AG.chronic.append("diab")

       # elif chance_hypertension >= hyps_chance:
          #  AG.chronic.append("hyp")

          #  if(chance_diabetes * chance_hypertension) >= hyps_chance:
          #      AG.chronic.append("diab")

        #elif chance_diabetes >= diab_chance:
         #    AG.chronic.append("diab")
    ...
    def update(self, agent):
        ...
        if agent.status == Status.Infected:
            ...
            if len(agent.chronic) == 0:
                if (0.0589/2) > death_test:
                    agent.status = Status.Death
                    agent.infected_status = InfectionSeverity.Asymptomatic
                    return

            if len(agent.chronic) == 1:
                if (0.135/2) > death_test:
                    agent.status = Status.Death
                    agent.infected_status = InfectionSeverity.Asymptomatic
                    return

            if len(agent.chronic) == 2:
                if (0.232/2) > death_test:
                    agent.status = Status.Death
                    agent.infected_status = InfectionSeverity.Asymptomatic
                    return

            if len(agent.chronic) == 3:
                if (0.329/2) > death_test:
                    agent.status = Status.Death
                    agent.infected_status = InfectionSeverity.Asymptomatic
                    return
